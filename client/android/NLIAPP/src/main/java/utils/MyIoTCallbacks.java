...
import com.ibm.watson.developer_cloud.android.myapplication.AppInfo;
...
public class MyIoTCallbacks implements IoTCallbacks {
    ...
    public void messageArrived(String topic, MqttMessage mqttMessage) throws Exception {
        ...
        String ressponse=top.getString("d");
        String empty="Something went wrong!";
        Log.d(TAG, ".messageArrived - Message received on topic " + topic
                + ": message is " + ressponse+"payload"+payload);

        if(ressponse!=null&&!ressponse.isEmpty()){
            app.setPayload(ressponse);
        }else  {
            app.setPayload(empty);
        }
